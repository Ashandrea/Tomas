            ...
            <div class="bg-white rounded-xl shadow-md overflow-hidden">
                <div class="p-6">
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                        ...
                        <a href="{{ route('foods.create2') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg transition-colors duration-200">
                            ...
                            @foreach($foods as $food)
                                <div class="border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-200">
                                    ...
                                    <div class="p-4">
                                        <div class="flex justify-between items-start">
                                            ...
                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium {{ $food->is_available ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                                {{ $food->is_available ? __('Tersedia') : __('Habis') }}
                                            </span>
                                        </div>
                                    ...
                                    <div class="px-4 py-3 bg-gray-50 border-t border-gray-100 flex justify-between">
                                        <form action="{{ route('foods.toggle', $food) }}" method="POST">
                                            @csrf
                                            @method('PATCH')
            ...
            <div class="bg-white rounded-xl shadow-md overflow-hidden">
                <div class="p-6">
                    <div class="flex flex-col md:flex-row justify-between items-center gap-4">
                        <div>
                            <h3 class="text-xl font-bold text-gray-800">{{ __('Layanan Pengiriman') }}</h3>
                            <p id="deliveryStatusText" class="text-sm mt-1 text-gray-500">
                                Memeriksa pesanan...
                            </p>
                        </div>
                        <a href="{{ route('courier2') }}" class="inline-flex items-center px-5 py-3 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-colors duration-200 relative" id="deliveryButton">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/>
                            </svg>
                            {{ __('Antarkan Pesanan') }}
                            <span id="notificationBadge" class="absolute -top-2 -right-2 inline-flex items-center justify-center h-6 w-6 rounded-full bg-red-500 text-xs font-medium text-white transform transition-all duration-200 ease-in-out hidden">
                                <span id="notificationCount" class="inline-block">0</span>
                            </span>
                        </a>
                    </div>
                </div>
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    function updateNotificationBadge(count) {
                        if (count > 0) {
                            ...
                            notificationBadge.classList.remove('hidden', 'scale-0');
                            notificationBadge.classList.add('animate-bounce-slow');
                            deliveryStatusText.innerHTML = `Ada <span class="font-medium text-green-600">${count} pesanan</span> yang perlu dikirim`;
                            
                            // Change animation to pulse after first bounce
                            setTimeout(() => {
                                notificationBadge.classList.remove('animate-bounce-slow');
                                notificationBadge.classList.add('animate-pulse');
                            }, 1500);
                        } else {
                            notificationBadge.classList.add('hidden');
                            deliveryStatusText.textContent = 'Belum ada pesanan yang perlu dikirim';
                        }
                    }
                    
                    // Initial check for ready orders
                    function checkReadyOrders() {
                        fetch('{{ route("orders.ready.count") }}', {
                            method: 'GET',
                            headers: {
                        ...
                        .catch(error => {
                            ...
                            deliveryStatusText.textContent = 'Gagal memuat status pesanan';
                        });
                    }
                    ...
                    setInterval(checkReadyOrders, 30000);
                    
                    // Add click handler to stop animation when clicked
